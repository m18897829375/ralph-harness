...
1. 合同提到某 skill → grep `skill-index.json` 确认存在
2. 合同提到某 CLI 工具 → grep `cli-index.json` 确认存在
3. 工具不存在于索引表 → 在退回理由中明确指出，建议改用索引表中已有的替代工具
4. 合同遗漏了明显可用的工具 → 在退回理由中建议添加

**示例退回理由：**
> "合同 verificationSteps 第3条引用 'puppeteer'，但 cli-index.json 中无此工具。
> 建议改用 playwright（browser-automation 类别，cli-index.json 已收录）。"
...
开始评估前，按合同内容搜索索引表选择验证工具：

1. **搜索 evaluator 阶段的技能：**
   `grep '"phase": "evaluator"' skill-index.json | grep -i '<contract keyword>'`
   例如：安全测试故事 → `grep '"phase": "evaluator"' skill-index.json | grep -i 'security'`

2. **搜索 CLI 验证工具：**
   `grep -E '"test-runner"|"browser-automation"|"api-client"|"code-quality"' cli-index.json`

3. **按类别选择：**
   - 浏览器 UI 测试 → `browser-automation` (playwright)
   - API 验证 → `api-client` (curl)
   - 类型检查 → `build-tool` (tsc)
   - 代码规范 → `linter-formatter` (eslint)
   - 安全扫描 → `agent-ops` (ecc-agentshield)
