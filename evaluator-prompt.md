...
1. **按验收标准选择工具。** 阅读验收标准，判断需要什么工具来验证。浏览器 UI 测试 → 使用已配置的浏览器 MCP 工具（如 Playwright MCP）。API 测试 → curl/httpie。数据库验证 → 对应 CLI。**禁止用"读代码判断"代替实测。**
...
5. **缺少工具的情况下给分 = 无效评估。** 因为验收标准未实测，评分无意义。
...
如果上下文中有 "SEARCH INDEX" 部分，**[REQUIRED] 必须使用** `scripts/search_index.py` 搜索索引表。评估前至少执行 2 次搜索。
...
**强制要求：审查合同前，必须用 search_index.py 验证 Generator 引用的所有工具真实存在于索引表中。跳过此步骤 = 合同审查不完整。**

1. 合同提到某 skill → `python3 scripts/search_index.py --type skill --name "<skill名>"`
2. 合同提到某 CLI 工具 → `python3 scripts/search_index.py --type cli --name "<工具名>"`
3. 工具不存在于索引表 → 在退回理由中明确指出，建议改用索引表中已有的替代工具
4. 合同遗漏了明显可用的工具 → `python3 scripts/search_index.py --type cli --keyword "<功能>"` 查找替代

**示例退回理由：**
> "合同 verificationSteps 第3条引用 'puppeteer'，但 search_index.py 未找到此工具。
> 建议改用 playwright（browser-automation 类别，已收录）。"
...
### [REQUIRED] 评估工具选择（从索引表查找）

**强制要求：开始评估前，必须至少执行 2 次 search_index.py 搜索。未搜索 = 工具准备不充分 = 评估不完整。**

开始评估前，按合同内容用 search_index.py 选择验证工具：

1. **搜索 evaluator 阶段的技能：**
   `python3 scripts/search_index.py --type skill --keyword "<合同关键词>" --phase "evaluator"`
   例如：安全测试故事 → `python3 scripts/search_index.py --type skill --keyword "security" --phase "evaluator"`

2. **搜索 CLI 验证工具：**
   `python3 scripts/search_index.py --type cli --keyword "test"` — 搜索 CLI
   `python3 scripts/search_index.py --type skill --keyword "<kw>" --category "testing"` — 搜索测试类技能

3. **按类别选择：**
   - 浏览器 UI 测试 → `--type skill --keyword "browser" --phase "evaluator"`
   - 安全扫描 → `--type skill --keyword "security" --phase "evaluator" --category "security"`
   - 性能分析 → `--type skill --keyword "performance" --phase "evaluator"`
   - 代码审查 → `--type skill --name "code-reviewer"`

4. **搜索 MCP 服务器（如需）：**
   `python3 scripts/search_index.py --type mcp --keyword "<功能>"`

5. **BM25 语义搜索技能（关键词搜索不佳时）：**
   `python3 scripts/match_skills.py "<自然语言查询>" --json`

---

### Subagent 调用（建议）

以下 subagent 可帮助提高评估质量，建议在评估时按需调用：

1. **code-reviewer**：审查 Generator 代码变更的质量，问题清单可作为 codeQuality 评分参考
2. **security-reviewer**：如涉及安全敏感代码建议调用，发现问题可作为扣分依据
3. **e2e-runner**：如涉及 UI 交互且已配置 Playwright MCP，建议执行自动化测试

调用结果记录在 evaluation.json 的 feedback 中。
